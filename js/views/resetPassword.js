...
export function renderResetPassword(container, { modo } = {}) {
  ...
  container.innerHTML = `
    <div class="splash" style="min-height:22vh">
      ...
      <h1>NutriRuta</h1>
      <p>${esReset ? 'Vamos a crear tu nueva contraseña.' : 'Ya casi está lista tu cuenta.'}</p>
    </div>
    <div class="card">
      <h2>${esReset ? 'Nueva contraseña' : 'Crea tu contraseña'}</h2>
      <p class="small muted mt">${esReset ? 'Elige una contraseña nueva para entrar a tu cuenta.' : 'Tu compra ya está confirmada. Con esto activas tu acceso.'}</p>
      <form novalidate>
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      toast(esReset ? 'Contraseña actualizada. ¡Bienvenida de vuelta! 🌿' : '¡Contraseña creada! Bienvenida a NutriRuta 🌿');
