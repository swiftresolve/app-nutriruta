...
export function renderResetPassword(container, { modo } = {}) {
  ...
  container.innerHTML = `
    <div class="splash" style="min-height:22vh">
      ...
      <h1>NutriRuta</h1>
      <p>${esReset ? t('Vamos a crear tu nueva contraseña.') : t('Ya casi está lista tu cuenta.')}</p>
    </div>
    <div class="card">
      <h2>${esReset ? t('Nueva contraseña') : t('Crea tu contraseña')}</h2>
      <p class="small muted mt">${esReset ? t('Elige una contraseña nueva para entrar a tu cuenta.') : t('Tu compra ya está confirmada. Con esto activas tu acceso.')}</p>
      <form novalidate>
        <label class="muted small" for="rp-pass">${t('Nueva contraseña')}</label>
        <input id="rp-pass" type="password" required minlength="${MIN_PASSWORD}" autocomplete="new-password" class="auth-input">
        <p class="small muted">${t('Mínimo {n} caracteres, con letras y números.', { n: MIN_PASSWORD })}</p>
        <p class="small" id="rp-error" style="color:var(--red);min-height:1.2em" role="alert"></p>
        <button class="btn full" type="submit">${t('Guardar y entrar')}</button>
      </form>
    </div>
    <div class="legal-note">${t('Si este enlace ya expiró, entra con "¿Olvidaste tu contraseña?" desde la pantalla de inicio de sesión.')}</div>`;
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      toast(esReset ? t('Contraseña actualizada. ¡Bienvenida de vuelta! 🌿') : t('¡Contraseña creada! Bienvenida a NutriRuta 🌿'));
      navigate(getState().onboarded ? 'dashboard' : 'quiz');
    } catch (err) {
      errEl.textContent = t('El enlace expiró o ya se usó. Pide uno nuevo con "¿Olvidaste tu contraseña?".');
      btn.disabled = false;
      btn.textContent = t('Guardar y entrar');
    }
  });
}
