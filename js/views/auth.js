...
export function passwordIssues(pw) {
  const issues = [];
  if (pw.length < MIN_PASSWORD) issues.push(t('mínimo {n} caracteres', { n: MIN_PASSWORD }));
  if (!/[a-záéíóúñ]/i.test(pw)) issues.push(t('al menos una letra'));
  if (!/\d/.test(pw)) issues.push(t('al menos un número'));
  return issues;
}
...
export function attachPasswordToggle(input) {
  ...
  btn.setAttribute('aria-label', t('Mostrar contraseña'));
  ...
  btn.addEventListener('click', () => {
    ...
    btn.setAttribute('aria-label', show ? t('Ocultar contraseña') : t('Mostrar contraseña'));
  ...
  function draw() {
    ...
    view.innerHTML = `
      <div class="splash" style="min-height:22vh">
        ...
        <h1>NutriRuta</h1>
        <p>${t('Tu ruta hacia hábitos saludables, paso a paso.')}</p>
      </div>
      <div class="card">
        <h2 class="center" style="justify-content:center">${mode === 'login' ? t('Inicia sesión') : t('Crea tu cuenta')}</h2>
        <button type="button" class="btn ghost full" id="a-google" style="display:flex;align-items:center;justify-content:center;gap:10px">${GOOGLE_ICON}${t('Continuar con Google')}</button>
        <div class="row center small muted mt" style="gap:8px"><span style="flex:1;height:1px;background:var(--primary-soft)"></span>${t('o')}<span style="flex:1;height:1px;background:var(--primary-soft)"></span></div>
        <form novalidate>
          ${mode === 'signup' ? `
          <label class="muted small" for="a-nombre">${t('Nombre o alias (opcional)')}</label>
          <input id="a-nombre" type="text" maxlength="60" autocomplete="nickname" class="auth-input">` : ''}
          <label class="muted small" for="a-email">${t('Correo electrónico')}</label>
          <input id="a-email" type="email" required autocomplete="email" class="auth-input" inputmode="email">
          <label class="muted small" for="a-pass">${t('Contraseña')}</label>
          ...
          ${mode === 'signup' ? `<p class="small muted">${t('Mínimo 8 caracteres, con letras y números.')}</p>` : ''}
          ${mode === 'login' ? `<p class="small" style="margin:-6px 0 6px"><button type="button" class="link-btn small" id="a-forgot">${t('¿Olvidaste tu contraseña?')}</button></p>` : ''}
          ...
          <button class="btn full" type="submit">${mode === 'login' ? t('Entrar') : t('Registrarme')}</button>
        </form>
        <p class="center mt small">
          ${mode === 'login' ? t('¿Primera vez aquí?') : t('¿Ya tienes cuenta?')}
          <button class="link-btn" id="a-toggle">${mode === 'login' ? t('Crea tu cuenta') : t('Inicia sesión')}</button>
        </p>
      </div>
      <div class="legal-note">${t('NutriRuta es una guía de autoayuda: no reemplaza la atención de un profesional de salud.')}</div>`;
    ...
    view.querySelector('#a-google').addEventListener('click', async () => {
      ...
      try {
        ...
        errEl.textContent = t('No se pudo continuar con Google. Intenta con correo y contraseña.');
        btn.disabled = false;
      }
    });
    ...
    view.querySelector('#a-forgot')?.addEventListener('click', async () => {
      ...
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        errEl.textContent = t('Escribe primero tu correo arriba, para saber a dónde enviarte el enlace.');
        return;
      }
      ...
      infoEl.textContent = t('Si {email} tiene una cuenta, te enviamos un enlace para crear una contraseña nueva. Revisa tu correo (y spam).', { email });
      link.disabled = false;
    });
    ...
    view.querySelector('form').addEventListener('submit', async (e) => {
      ...
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { errEl.textContent = t('Escribe un correo válido.'); return; }
      if (mode === 'signup') {
        const issues = passwordIssues(pass);
        if (issues.length) { errEl.textContent = t('Contraseña insegura: ') + issues.join(', ') + '.'; return; }
      }

      const btn = view.querySelector('.btn');
      btn.disabled = true; btn.textContent = t('Un momento…');
      ...
      if (mode === 'signup') {
        ...
        container.innerHTML = `
          <div class="card center" style="margin-top:22vh">
            ...
            <h2 class="mt">${t('Creando tu cuenta…')}</h2>
            <p class="small muted mt">${t('Espera un momento')}</p>
          </div>`;
      }
      ...
      try {
          ...
          if (!data.session) {
            container.innerHTML = `
              <div class="card center" style="margin-top:20vh">
                ...
                <h2>${t('Confirma tu correo')}</h2>
                <p class="mt">${t('Te enviamos un enlace a {strong}{email}{fin}. Ábrelo para activar tu cuenta y luego inicia sesión.', { strong: '<strong>', email: email.replace(/</g, '&lt;'), fin: '</strong>' })}</p>
                <button class="btn full mt" id="a-back">${t('Volver a iniciar sesión')}</button>
              </div>`;
        ...
        setAuthed(true);
        await initCloud();
        toast(t('¡Qué gusto tenerte en NutriRuta! 🌿'));
        ...
        if (mode === 'login') { btn.disabled = false; btn.textContent = t('Entrar'); }
      }
    });
...
function friendlyError(err) {
  ...
  if (/invalid login credentials/i.test(msg)) return t('Correo o contraseña incorrectos.');
  if (/already registered/i.test(msg)) return t('Ese correo ya tiene una cuenta. Inicia sesión.');
  if (/email not confirmed/i.test(msg)) return t('Debes confirmar tu correo antes de entrar. Revisa tu bandeja.');
  if (/password should be/i.test(msg)) return t('La contraseña no cumple los requisitos mínimos.');
  if (/rate limit|too many/i.test(msg)) return t('Demasiados intentos. Espera un momento e inténtalo de nuevo.');
  if (/failed to fetch|network/i.test(msg)) return t('Sin conexión. Verifica tu internet e inténtalo de nuevo.');
  return t('No se pudo completar. Inténtalo de nuevo.');
}
